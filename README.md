...
Since `chai-tailwind` is currently not published on npm, you need to build it manually to use it in your projects.

1. **Clone the repository:**
```bash
git clone https://github.com/satyasootar/chai-tailwind.git
cd chai-tailwind
```

2. **Install dependencies and build the engine:**
```bash
npm install
npm run build
```
...
Copy the generated file from the `dist/` directory into your project and link it via a `<script>` tag.

```html
<!-- Auto-initializes on DOMContentLoaded -->
<script src="path/to/your/dist/index.global.js"></script>
```
